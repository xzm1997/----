...
- 非简单请求：凡是不同时满足上面两个条件，就属于非简单请求。即可能对服务器数据产生副作用的HTTP请求方法，如put，delete都会对服务器数据进行更修改，所以要先询问服务器
...
- 现在执行：将代码转为字节码
  ...
  - 首先，解释器需要将我们写的源代码生成AST语法树和执行上下文, 先对源代码进行分词,进行词法分析就是将代码拆分成token,token就是指语法上不可再分的最小单位字符或者字符串，之后进行解析做语法分析生成AST,有了AST和执行上下文之后，解释器根据AST生成字节码，然后执行。
- 过去执行：将代码转为机器码，又什么不足
- 执行代码中的优化
