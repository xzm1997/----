const read_line = require('readline-sync')
let input = read_line.question("")

let numQs, numRev, mark = BigInt(0), upList = [];
[numQs, numRev] = input.split(' ')
numQs = parseInt(numQs);
numRev = parseInt(numRev)

let pList = read_line.question("").split(' ').map(item => BigInt(parseInt(item)));
let markList = read_line.question("").split(' ').map(item => BigInt(parseInt(item)));

for (let i = 0; i < numQs; ++i) {
  mark += (pList[i] * markList[i]);
}

for (let i = 0; i < numQs; ++i) {
  let temp = (BigInt(100) - pList[i]) * markList[i];
  upList.push(temp);
}

upList.sort((a, b) => b - a);
for (let i = 0; i < numRev; ++i) {
  mark += upList[i];
}

console.log(mark)
