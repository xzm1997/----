...
for (let i = 0; i < num; ++i) {
  for (let j = i + 1; j < num; ++j) {
    for (let k = j + 1; k < num; ++k) {
      ...
      // }
      let square = [];
      square.push(Math.sqrt(dir[i][0] - dir[j][0], 2) + Math.sqrt(dir[i][1] - dir[k][1], 2));
      square.push(Math.sqrt(dir[i][0] - dir[j][0], 2) + Math.sqrt(dir[j][1] - dir[k][1], 2));
      square.push(Math.sqrt(dir[j][0] - dir[j][0], 2) + Math.sqrt(dir[i][1] - dir[k][1], 2));
      square.sort((a, b) => a-b);
      if (square[0] === square[2] + square[1]) ++res;
    }
  }
}
